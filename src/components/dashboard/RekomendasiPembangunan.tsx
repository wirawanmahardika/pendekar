...
export default function RekomendasiPembangunan({ resultData }: { resultData?: dashboardResultDataType }) {
  ...
  useEffect(() => {
    if (resultData?.list_desa) {
      ...
      if (selectedKecamatan) {
        filtered = filtered.filter(desa => 
          desa.nama_kecamatan === selectedKecamatan
        );
      }
      
      setFilteredDesa(filtered);
    }
  }, [resultData, searchTerm, selectedKecamatan]);
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (statusLower.includes("mandiri")) {
      return <div className="badge bg-emerald-50 text-emerald-500 border-none">Mandiri</div>;
    } else if (statusLower.includes("maju")) {
      return <div className="badge bg-blue-50 text-blue-500 border-none">Maju</div>;
    } else if (statusLower.includes("berkembang")) {
      return <div className="badge bg-sky-50 text-sky-500 border-none">Berkembang</div>;
    } else if (statusLower.includes("tertinggal")) {
      return <div className="badge bg-orange-50 text-orange-500 border-none">Tertinggal</div>;
    } else if (statusLower.includes("sangat tertinggal")) {
      return <div className="badge bg-red-50 text-red-500 border-none">Sangat Tertinggal</div>;
    }
    
    return <div className="badge bg-gray-50 text-gray-500 border-none">{status}</div>;
  };
  ...
  return (
    <div className="p-4 bg-white rounded shadow mt-8">
      ...
      <div className="flex gap-x-5 pt-2">
        ...
        <select className="focus:border-blue-400 focus:shadow border text-sm border-slate-300 rounded text-neutral-600 w-1/4 outline-none pl-2 pr-4 py-1">
          <option value="">Semua Desa</option>
          {filteredDesa.map((desa, index) => (
            <option key={index} value={desa.nama_deskel}>{desa.nama_deskel}</option>
          ))}
        </select>
      </div>
      ...
      <table className="overflow-x-auto min-w-full mt-5 text-neutral-700">
        ...
        <tbody>
          {currentItems.map((desa, index) => (
            <tr key={index}>
              <td className="p-2 border border-gray-300">{indexOfFirstItem + index + 1}</td>
          ...
          {currentItems.length === 0 && (
            <tr>
              <td colSpan={7} className="p-4 text-center border border-gray-300">
