...
export default function JenisUsaha({ resultData }: { resultData?: umkmDataType }) {
    ...
    const onChangeFilter =async (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    return <div className="bg-white rounded p-4 flex flex-col h-[600px]">
        ...
        <div className="flex gap-x-5 pt-2">
            <div className="flex relative">
                <input name="text" onChange={onChangeFilter} type="text" placeholder="Cari Desa/Kelurahan..." className="focus:border-blue-400 focus:shadow border text-sm border-slate-300 rounded text-neutral-600 w-full outline-none pl-2 pr-10 py-1" />
                <BiSearch className="absolute right-2 top-1/2 -translate-y-1/2 size-6 fill-slate-600" />
            </div>

            <select name="kecamatan" onChange={onChangeFilter} className="focus:border-blue-400 focus:shadow border text-sm border-slate-300 rounded text-neutral-600 w-1/4 outline-none pl-2 pr-4 py-1">
                <option value="">Semua Kecamatan</option>
                {resultData?.list_kecamatan.map(d => <option key={d.kode_wilayah} value={d.k3}>{d.nama_kecamatan}</option>)}
            </select>

            <select name="desa" onChange={onChangeFilter} className="focus:border-blue-400 focus:shadow border text-sm border-slate-300 rounded text-neutral-600 w-1/4 outline-none pl-2 pr-4 py-1">
